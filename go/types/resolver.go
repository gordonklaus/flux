...
func (check *checker) reportAltDecl(obj Object) {
	if pos := obj.Pos(); pos.IsValid() {
		...
		check.errorf(pos, "\tother declaration of %s", obj.GetName()) // secondary error, \t indented
	}
}

func (check *checker) declare(scope *Scope, id *ast.Ident, obj Object) {
	if alt := scope.Insert(obj); alt != nil {
		check.errorf(obj.Pos(), "%s redeclared in this block", obj.GetName())
...
func (check *checker) dependencies(obj Object, init *declInfo, path []Object) {
	...
	if init.mark > 0 {
		...
		if first >= 0 {
			obj := cycle[first]
			check.errorf(obj.Pos(), "initialization cycle for %s", obj.GetName())
			// print cycle
			i := first
			for _ = range cycle {
				check.errorf(obj.Pos(), "\t%s refers to", obj.GetName()) // secondary error, \t indented
				i++
				if i >= len(cycle) {
					i = 0
				}
			...
			check.errorf(obj.Pos(), "\t%s (cycle start)", obj.GetName())
...
func (check *checker) varDecl(obj *Var, lhs []*Var, typ, init ast.Expr) {
	...
	// determine type, if any
	if typ != nil {
		obj.Type = check.typ(typ, nil, false)
	}
...
func (check *checker) typeDecl(obj *TypeName, typ ast.Expr, def *Named, cycleOk bool) {
	assert(obj.GetType() == nil)

	// type declarations cannot use iota
	assert(check.iota == nil)

	named := &Named{Obj: obj}
	obj.Type = named // make sure recursive type declarations terminate
	...
	if t, _ := named.UnderlyingT.(*Struct); t != nil {
...
func (check *checker) funcDecl(obj *Func, info *declInfo) {
	...
	obj.Type = Typ[Invalid] // guard against cycles
	fdecl := info.fdecl
	sig := check.funcType(fdecl.Recv, fdecl.Type, nil)
	if sig.Recv == nil && obj.Name == "init" && (len(sig.Params) > 0 || len(sig.Results) > 0) {
