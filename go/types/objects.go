...
func NewLabel(pos token.Pos, name string) *Label {
	return &Label{object{pos: pos, Name: name, Type: Typ[Invalid]}}
}
...
func newBuiltin(id builtinId) *Builtin {
	return &Builtin{object{Name: predeclaredFuncs[id].name, Type: Typ[Invalid]}, id}
}
...
func writeObject(buf *bytes.Buffer, this *Package, obj Object) {
	...
	if pkg := obj.GetPkg(); pkg != nil && this != pkg && pkg.scope.Lookup(obj.GetName()) == obj {
...
func writeFuncName(buf *bytes.Buffer, this *Package, f *Func) {
	if f.Type != nil {
		...
		if recv := sig.Recv; recv != nil {
			buf.WriteByte('(')
			if _, ok := recv.GetType().(*Interface); ok {
				...
				WriteType(buf, this, recv.GetType())
			}
			buf.WriteByte(')')
			buf.WriteByte('.')
