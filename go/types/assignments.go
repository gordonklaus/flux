...
func (check *checker) assignment(x *operand, T Type) bool {
	...
	if t, _ := x.typ.(*Tuple); t != nil {
		assert(len(*t) > 1)
		check.errorf(x.pos(), "%d-valued expression %s used as single value", len(*t), x)
		x.mode = invalid
		return false
	}
...
func (check *checker) initConst(lhs *Const, x *operand) {
	if x.mode == invalid || x.typ == Typ[Invalid] || lhs.Type == Typ[Invalid] {
		if lhs.Type == nil {
			lhs.Type = Typ[Invalid]
		}
		return
	}
	...
	if !check.assignment(x, lhs.Type) {
		if x.mode != invalid {
			check.errorf(x.pos(), "cannot define constant %s (type %s) as %s", lhs.GetName(), lhs.Type, x)
		}
...
func (check *checker) initVar(lhs *Var, x *operand) Type {
	...
	if !check.assignment(x, lhs.Type) {
		if x.mode != invalid {
			check.errorf(x.pos(), "cannot initialize variable %s (type %s) with %s", lhs.GetName(), lhs.Type, x)
		}
		return nil
	}
